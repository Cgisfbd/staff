...
const List<DocumentItem> kStandardAdmissionDocs = [
  DocumentItem(
    id: 'studentAadhar',
    title: "Student's Aadhaar",
...
/// Luxury Verification Checklist Section for Submitted Documents (< 160 lines).
/// Perfectly harmonized with the Champagne Gold & Velvet Obsidian design system.
class AdmissionDocumentsSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final totalCount = kStandardAdmissionDocs.length;
    ...
    return AdmissionSectionCard(
      ...
      child: Column(
        ...
        children: [
          ...
          for (int i = 0; i < 6; i += 2) ...[
            Row(
              children: [
                Expanded(
                  child: _buildDocChip(
                    doc: kStandardAdmissionDocs[i],
                    isChecked: selectedDocIds.contains(kStandardAdmissionDocs[i].id),
                    isDark: isDark,
                    onTap: () => onToggleDoc(kStandardAdmissionDocs[i].id),
                  ),
                ),
                const SizedBox(width: 8),
                Expanded(
                  child: _buildDocChip(
                    doc: kStandardAdmissionDocs[i + 1],
                    isChecked: selectedDocIds.contains(kStandardAdmissionDocs[i + 1].id),
                    isDark: isDark,
                    onTap: () => onToggleDoc(kStandardAdmissionDocs[i + 1].id),
                  ),
                ),
              ],
            ),
          ...
          // 7th Document: Other Document (Full-Width Chip)
          _buildDocChip(
            doc: kStandardAdmissionDocs[6],
            isChecked: selectedDocIds.contains(kStandardAdmissionDocs[6].id),
            isDark: isDark,
            onTap: () => onToggleDoc(kStandardAdmissionDocs[6].id),
          ),
          ...
          if (selectedDocIds.contains('otherDoc')) ...[
            ...
            AdmissionTextField(
              ...
              hint: 'e.g. Birth Certificate, Ration Card, Medical...',
