...
/// Ultra-Luxury Glassmorphic KPI Row strictly complying with:
...
class FeesKpiCards extends StatelessWidget {
  const FeesKpiCards({super.key, required this.kpi});
  ...
  Widget build(BuildContext context) {
    ...
    return SingleChildScrollView(
      ...
      child: Row(
        children: [
          // 1. Total Expected (10-Month Session) -> Champagne Gold
          _buildCard(
            title: 'Total Expected',
            badge: '10-Months',
            value: '₹${kpi.annualProjected.toInt().toString().replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}',
          ...
          // 2. Total Collected -> Emerald
          _buildCard(
            title: 'Total Collected',
            badge: '${kpi.collectionRate}% Paid',
            value: '₹${kpi.totalCollected.toInt().toString().replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}',
          ...
          // 3. Total Pending (Due Balance) -> Rose Crimson
          _buildCard(
            title: 'Total Pending',
            badge: 'Due Balance',
            value: '₹${kpi.totalPending.toInt().toString().replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}',
          ...
          // 4. Total Waived Off -> Warm Amber Gold (Zero Purple)
          _buildCard(
            title: 'Total Waived Off',
            badge: 'Concession',
            value: '₹${kpi.totalWaived.toInt().toString().replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}',
          ...
          // 5. Enrolled Students -> Muted Antique Gold
          _buildCard(
          ...
          // 6. Courses & Classes -> Cashmere Bronze
          _buildCard(
            title: 'Courses / Classes',
            badge: 'Curriculum',
            value: '${kpi.totalCourses} / ${kpi.totalClasses}',
