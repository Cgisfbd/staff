...
class BookExamSetupPage extends StatefulWidget {
  const BookExamSetupPage({
    super.key,
    required this.book,
  });
  ...
  @override
  State<BookExamSetupPage> createState() => _BookExamSetupPageState();
}

class _BookExamSetupPageState extends State<BookExamSetupPage> {
  ...
  void initState() {
    ...
    _passMarksController = TextEditingController(
      text: '${b.passMarks > 0 ? b.passMarks : (max * 0.33).round()}',
    );
  ...
  void _onTheoryChanged(int termNum, String val) {
    final newTheory = int.tryParse(val) ?? 0;
    final totalMax = int.tryParse(_maxMarksController.text) ?? 100;

    if (_term == 'TERM_1') {
      if (termNum == 1) {
        final t1Prac = _t1HasPractical ? (int.tryParse(_t1PracticalController.text) ?? 0) : 0;
        final t1Tot = newTheory + t1Prac;
        final t2Prac = _t2HasPractical ? (int.tryParse(_t2PracticalController.text) ?? 0) : 0;
        final remT2Theory = (totalMax - t1Tot - t2Prac).clamp(0, totalMax);
        _t2TheoryController.text = '$remT2Theory';
      } else if (termNum == 2) {
        final t2Prac = _t2HasPractical ? (int.tryParse(_t2PracticalController.text) ?? 0) : 0;
        final t2Tot = newTheory + t2Prac;
        final t1Prac = _t1HasPractical ? (int.tryParse(_t1PracticalController.text) ?? 0) : 0;
        final remT1Theory = (totalMax - t2Tot - t1Prac).clamp(0, totalMax);
        _t1TheoryController.text = '$remT1Theory';
      }
    } else if (_term == 'TERM_2') {
      if (termNum == 3) {
        final t3Prac = _t3HasPractical ? (int.tryParse(_t3PracticalController.text) ?? 0) : 0;
        final t3Tot = newTheory + t3Prac;
        final t4Prac = _t4HasPractical ? (int.tryParse(_t4PracticalController.text) ?? 0) : 0;
        final remT4Theory = (totalMax - t3Tot - t4Prac).clamp(0, totalMax);
        _t4TheoryController.text = '$remT4Theory';
      } else if (termNum == 4) {
        final t4Prac = _t4HasPractical ? (int.tryParse(_t4PracticalController.text) ?? 0) : 0;
        final t4Tot = newTheory + t4Prac;
        final t3Prac = _t3HasPractical ? (int.tryParse(_t3PracticalController.text) ?? 0) : 0;
        final remT3Theory = (totalMax - t4Tot - t3Prac).clamp(0, totalMax);
        _t3TheoryController.text = '$remT3Theory';
      }
    } else {
      // FULL_YEAR
      if (termNum == 4) {
        final t1Tot = _t1Total;
        final t3Tot = _t3Total;
        final t4Prac = _t4HasPractical ? (int.tryParse(_t4PracticalController.text) ?? 0) : 0;
        final t4Tot = newTheory + t4Prac;
        final t2Prac = _t2HasPractical ? (int.tryParse(_t2PracticalController.text) ?? 0) : 0;
        final remT2Theory = (totalMax - (t1Tot + t3Tot + t4Tot) - t2Prac).clamp(0, totalMax);
        _t2TheoryController.text = '$remT2Theory';
      } else {
        final t1Tot = termNum == 1 ? (newTheory + (_t1HasPractical ? (int.tryParse(_t1PracticalController.text) ?? 0) : 0)) : _t1Total;
        final t2Tot = termNum == 2 ? (newTheory + (_t2HasPractical ? (int.tryParse(_t2PracticalController.text) ?? 0) : 0)) : _t2Total;
        final t3Tot = termNum == 3 ? (newTheory + (_t3HasPractical ? (int.tryParse(_t3PracticalController.text) ?? 0) : 0)) : _t3Total;
        final t4Prac = _t4HasPractical ? (int.tryParse(_t4PracticalController.text) ?? 0) : 0;
        final remT4Theory = (totalMax - (t1Tot + t2Tot + t3Tot) - t4Prac).clamp(0, totalMax);
        _t4TheoryController.text = '$remT4Theory';
      }
    }
    _onRecalculate();
  ...
  void _onPracticalMarksChanged(int termNum, String val) {
    final newPrac = int.tryParse(val) ?? 0;
    final totalMax = int.tryParse(_maxMarksController.text) ?? 100;

    if (_term == 'TERM_1') {
      if (termNum == 1) {
        final t1Tot = (int.tryParse(_t1TheoryController.text) ?? 0) + newPrac;
        final t2Prac = _t2HasPractical ? (int.tryParse(_t2PracticalController.text) ?? 0) : 0;
        final remT2Theory = (totalMax - t1Tot - t2Prac).clamp(0, totalMax);
        _t2TheoryController.text = '$remT2Theory';
      } else if (termNum == 2) {
        final t2Tot = (int.tryParse(_t2TheoryController.text) ?? 0) + newPrac;
        final t1Prac = _t1HasPractical ? (int.tryParse(_t1PracticalController.text) ?? 0) : 0;
        final remT1Theory = (totalMax - t2Tot - t1Prac).clamp(0, totalMax);
        _t1TheoryController.text = '$remT1Theory';
      }
      ...
      if (termNum == 3) {
        final t3Tot = (int.tryParse(_t3TheoryController.text) ?? 0) + newPrac;
        final t4Prac = _t4HasPractical ? (int.tryParse(_t4PracticalController.text) ?? 0) : 0;
        final remT4Theory = (totalMax - t3Tot - t4Prac).clamp(0, totalMax);
        _t4TheoryController.text = '$remT4Theory';
      } else if (termNum == 4) {
        final t4Tot = (int.tryParse(_t4TheoryController.text) ?? 0) + newPrac;
        final t3Prac = _t3HasPractical ? (int.tryParse(_t3PracticalController.text) ?? 0) : 0;
        final remT3Theory = (totalMax - t4Tot - t3Prac).clamp(0, totalMax);
        _t3TheoryController.text = '$remT3Theory';
      }
      ...
      if (termNum == 4) {
        ...
        final t1Tot = termNum == 1 ? ((int.tryParse(_t1TheoryController.text) ?? 0) + newPrac) : _t1Total;
        final t2Tot = termNum == 2 ? ((int.tryParse(_t2TheoryController.text) ?? 0) + newPrac) : _t2Total;
        final t3Tot = termNum == 3 ? ((int.tryParse(_t3TheoryController.text) ?? 0) + newPrac) : _t3Total;
        final t4Prac = _t4HasPractical ? (int.tryParse(_t4PracticalController.text) ?? 0) : 0;
        final remT4Theory = (totalMax - (t1Tot + t2Tot + t3Tot) - t4Prac).clamp(0, totalMax);
        _t4TheoryController.text = '$remT4Theory';
      }
  ...
  void _togglePractical(int termNum, bool enable) {
    setState(() {
      switch (termNum) {
        case 1:
          _t1HasPractical = enable;
          if (enable) {
            final curPrac = int.tryParse(_t1PracticalController.text) ?? 0;
            if (curPrac == 0) {
              final curTheory = int.tryParse(_t1TheoryController.text) ?? 0;
              final practicalDefault = curTheory >= 10 ? 10 : (curTheory > 0 ? (curTheory / 2).round() : 5);
              _t1PracticalController.text = '$practicalDefault';
              _t1TheoryController.text = '${(curTheory - practicalDefault).clamp(0, 9999)}';
            }
          } else {
            final curPrac = int.tryParse(_t1PracticalController.text) ?? 0;
            final curTheory = int.tryParse(_t1TheoryController.text) ?? 0;
            _t1TheoryController.text = '${curTheory + curPrac}';
            _t1PracticalController.text = '0';
          }
          break;
        case 2:
          _t2HasPractical = enable;
          if (enable) {
            final curPrac = int.tryParse(_t2PracticalController.text) ?? 0;
            if (curPrac == 0) {
              final curTheory = int.tryParse(_t2TheoryController.text) ?? 0;
              final practicalDefault = curTheory >= 10 ? 10 : (curTheory > 0 ? (curTheory / 2).round() : 10);
              _t2PracticalController.text = '$practicalDefault';
              _t2TheoryController.text = '${(curTheory - practicalDefault).clamp(0, 9999)}';
            }
          } else {
            final curPrac = int.tryParse(_t2PracticalController.text) ?? 0;
            final curTheory = int.tryParse(_t2TheoryController.text) ?? 0;
            _t2TheoryController.text = '${curTheory + curPrac}';
            _t2PracticalController.text = '0';
          }
          break;
        case 3:
          _t3HasPractical = enable;
          if (enable) {
            final curPrac = int.tryParse(_t3PracticalController.text) ?? 0;
            if (curPrac == 0) {
              final curTheory = int.tryParse(_t3TheoryController.text) ?? 0;
              final practicalDefault = curTheory >= 10 ? 10 : (curTheory > 0 ? (curTheory / 2).round() : 5);
              _t3PracticalController.text = '$practicalDefault';
              _t3TheoryController.text = '${(curTheory - practicalDefault).clamp(0, 9999)}';
            }
          } else {
            final curPrac = int.tryParse(_t3PracticalController.text) ?? 0;
            final curTheory = int.tryParse(_t3TheoryController.text) ?? 0;
            _t3TheoryController.text = '${curTheory + curPrac}';
            _t3PracticalController.text = '0';
          }
          break;
        case 4:
          _t4HasPractical = enable;
          if (enable) {
            final curPrac = int.tryParse(_t4PracticalController.text) ?? 0;
            if (curPrac == 0) {
              final curTheory = int.tryParse(_t4TheoryController.text) ?? 0;
              final practicalDefault = curTheory >= 10 ? 10 : (curTheory > 0 ? (curTheory / 2).round() : 10);
              _t4PracticalController.text = '$practicalDefault';
              _t4TheoryController.text = '${(curTheory - practicalDefault).clamp(0, 9999)}';
            }
          } else {
            final curPrac = int.tryParse(_t4PracticalController.text) ?? 0;
            final curTheory = int.tryParse(_t4TheoryController.text) ?? 0;
            _t4TheoryController.text = '${curTheory + curPrac}';
            _t4PracticalController.text = '0';
          }
          break;
      }
  ...
  void _submit() {
    ...
    context.read<ExamSettingsBloc>().add(
          UpdateBookExamSettingsEvent(
            book: widget.book,
            term: _term,
            maxMarks: maxMarks,
            passMarks: passMarks,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: AppBackground(
        child: Column(
          children: [
            // Top Executive Header with Back Arrow
            ExecutiveTopHeader(
              icon: Icons.arrow_back_rounded,
              title: 'Exam Setup',
              subtitle: widget.book.nameEnglish,
                ...
                child: Form(
                  ...
                  child: Column(
                    ...
                    children: [
                                ...
                                children: [
                                  Row(
                                    children: [
                                      ...
                                      if (widget.book.nameUrdu.isNotEmpty) ...[
                                        const SizedBox(width: 6),
                                        Text(
                                          widget.book.nameUrdu,
                                  ...
                                  Text(
                                    'Subject: ${(widget.book.subjectName?.isNotEmpty == true) ? widget.book.subjectName! : 'Assigned Subject'} • ${widget.book.category == 'ASRI' ? 'Asri' : 'Deeni'}',
                                    style: TextStyle(
                                      fontSize: 11,
                      ...
                      // 1. TOP SECTION: OVERALL MARKS FRAMEWORK
                      Container(
                        padding: const EdgeInsets.all(12),
                        decoration: BoxDecoration(
                          ...
                          Container(
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                Text(
                                  isMatched
                                      ? '⚡ Live Balanced: $_totalMaxMarks / $maxTarget (Matched)'
                      ...
                      _TermCardWidget(
                        ...
                        onPracticalChanged: (val) => _togglePractical(1, val),
                        onTheoryChanged: (val) => _onTheoryChanged(1, val),
                        onPracticalMarksChanged: (val) => _onPracticalMarksChanged(1, val),
                        totalMarks: _t1Total,
                        isDark: isDark,
                      ),
                      const SizedBox(height: 8),
                      ...
                      _TermCardWidget(
                        ...
                        onPracticalChanged: (val) => _togglePractical(2, val),
                        onTheoryChanged: (val) => _onTheoryChanged(2, val),
                        onPracticalMarksChanged: (val) => _onPracticalMarksChanged(2, val),
                        totalMarks: _t2Total,
                        isDark: isDark,
                      ),
                      const SizedBox(height: 8),
                      ...
                      _TermCardWidget(
                        ...
                        onPracticalChanged: (val) => _togglePractical(3, val),
                        onTheoryChanged: (val) => _onTheoryChanged(3, val),
                        onPracticalMarksChanged: (val) => _onPracticalMarksChanged(3, val),
                        totalMarks: _t3Total,
                        isDark: isDark,
                      ),
                      const SizedBox(height: 8),
                      ...
                      _TermCardWidget(
                        ...
                        onPracticalChanged: (val) => _togglePractical(4, val),
                        onTheoryChanged: (val) => _onTheoryChanged(4, val),
                        onPracticalMarksChanged: (val) => _onPracticalMarksChanged(4, val),
                        totalMarks: _t4Total,
                        isDark: isDark,
                      ),
...
class _TermCardWidget extends StatelessWidget {
  const _TermCardWidget({
    ...
    required this.onPracticalMarksChanged,
  ...
  final ValueChanged<String> onPracticalMarksChanged;
                        ...
                        children: [
                          ...
                          Container(
                            ...
                            child: Row(
                              ...
                              children: [
                                InkWell(
                                  onTap: () => onPracticalChanged(false),
                                  borderRadius: BorderRadius.circular(6),
                                  child: Container(
                                ...
                                InkWell(
                                  onTap: () => onPracticalChanged(true),
                                  borderRadius: BorderRadius.circular(6),
                                  child: Container(
                                    padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
                                    decoration: BoxDecoration(
                                      color: hasPractical ? Colors.amber : Colors.transparent,
                                      borderRadius: BorderRadius.circular(6),
                                      boxShadow: hasPractical
                                          ? [
                                              BoxShadow(
                                    ...
                                    child: Text(
                                      ...
                                      style: TextStyle(
                                        fontSize: 10,
                                        fontWeight: FontWeight.bold,
                                        color: hasPractical ? Colors.white : Colors.grey,
                                      ),
                                    ),
                                  ),
                                ),
                    ...
                    if (hasPractical)
                      SizedBox(
                        ...
                        child: TextFormField(
                          ...
                          keyboardType: TextInputType.number,
                          onChanged: onPracticalMarksChanged,
                          style: const TextStyle(
                            fontSize: 12,
                            fontWeight: FontWeight.bold,
                            color: Colors.amber,
